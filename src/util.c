...
dcVersion * dc_version_new(
  ...
  /* note that only buf needs to be freed, since everything else references
   * some location within buf.
   */
  char *buf, *ptr;
  char *version = NULL;
  char *revision = NULL;
  ...
  while (*ptr != '\0')
  {
    ...
    {
      *ptr++ = '\0';
      vobj->epoch.string = strdup(buf);
      break;
    }
    ptr++;
  }

  /* if there was no epoch, ptr will be NUL, so re-start */
  if (*ptr == '\0')
    ptr = buf;
  ...
  /* if a revision was found, the character immediately preceding it must
   * be a '-', and must be transformed to a NUL byte (to terminate version)
   */
  if (revision != NULL)
    *(revision-1) = '\0';
  ...
  if (version == NULL)
    ...
    vobj->version = strdup(version);

  if (revision == NULL)
    vobj->revision = NULL;
  else
    vobj->revision = strdup(revision);
...
void dc_version_free(
  ...
  if (!(*ptr)->valid && (*ptr)->epoch.string != NULL)
  {
    free((*ptr)->epoch.string);
  }
